...
Original file is located at
    https://colab.research.google.com/drive/1ijm82wd4dDAv7cXd2YSdZNgsPEuRUeJs
...
def prob(x, mu, sigma, n):
  ...
  # P(X>1400)
  z = (x - mu)/(sigma/np.sqrt(n))
  probabilidade = ss.distributions.norm.sf(z)
...
def ic(dados):
  ...
  x = dados.mean()
  sigma = np.sqrt(np.var(dados))
  li = x - (1.96*sigma)/np.sqrt(dados.size)
  ls = x + (1.96*sigma)/np.sqrt(dados.size)
  return li, ls
...
def t_obs(data):
  ...
  data_auxiliar["filtro"] = data_auxiliar["floors"] == 1.0
  data_auxiliar = data_auxiliar.groupby("filtro").mean()
  resultado = data_auxiliar["price"][1] - data_auxiliar["price"][0]
...
def shuffling(data):
  ...
  for i in range(5000):
    ...
    diferencas[i] = data[filtro]["price"].mean() - data[~filtro]["price"].mean()

  li = np.percentile(diferencas,2.5)
  ls = np.percentile(diferencas,97.5)
