...
Original file is located at
    https://colab.research.google.com/drive/1O0Unqsowx3SCtIpIY-cLCfS10TZD5aVh
...
def percent(df1, df2, column, alpha):
  ...
  if (np.percentile(values_diff, 100*alpha) < 0 and np.percentile(values_diff, 100 - 100*alpha)>0):
    resultado = True
...
def t_obs(data):
  ...
  df_auxiliar = data.groupby('TEAM').mean()
  resultado = df_auxiliar["SALARY"][1] - df_auxiliar["SALARY"][0]
...
def resposta(data):
  ...
  filtro = data["TEAM"] == "Charlotte Hornets"
  diferencas = np.zeros(10000)

  for i in range(10000):
    np.random.shuffle(filtro.values)
    diferencas[i] = data[~filtro]["SALARY"].mean() - data[filtro]["SALARY"].mean()

  if np.percentile(diferencas, (0.10/2)*100).round(2) < -1.70 and np.percentile(diferencas, (1 - 0.10/2)*100).round(2) > -1.70:
    resultado = True
  else:
    resultado = False
...
def t_obs2(data):
  ...
  df_auxiliar = data.groupby('team_abbreviation').mean()
  resultado = df_auxiliar["player_height"][1] - df_auxiliar["player_height"][0]
...
def resposta2(diffs, t_obs):
  ...
  if np.percentile(diffs, 2.5).round(2) < t_obs and np.percentile(diffs, 97.5).round(2) > t_obs:
    resultado = False
  else:
    resultado = True
