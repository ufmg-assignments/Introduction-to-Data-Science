...
Original file is located at
    https://colab.research.google.com/drive/1cOQlAHoECoEVqtRfu2VQvNY_hCwLallV
...
def minimize_batch(param, lambda_=0.0001, niter=1000000):
  ...
  for i in range(niter):
      ...
      parametros_novos = np.array([x, y]) - lambda_ * grad

      x, y = parametros_novos
...
assert_equal(np.round(minimo, 4), -3.0)
assert_equal(np.round(x_min, 4), -4.0)
assert_equal(np.round(y_min, 4), 2.0)
...
def z_norm(dataset):
    ...
    dataset_aux = dataset_aux - dataset_aux.mean()
    dataset_aux = dataset_aux/dataset_aux.std(ddof=1)
...
def gradient(x_i, y_i, alpha, beta):
  ...
  # deve retornar uma lista [da, db]
    ...
    da =  2 * beta * x_i -2 * y_i + 2 * alpha
    db = 2 * alpha * x_i -2 * y_i * x_i + 2 * beta * (x_i ** 2)
...
def descent(x, y, param, lambda_=0.0001, niter=1000):
  ...
  for i in range(niter):
      ...
      while (contador != total_pontos):
        ...
        aux1, aux2 = gradient(x[contador], y[contador], a, b)
        d_a += aux1
        d_b += aux2
        contador+=1
...
def descent_vec(x, y, param, lambda_=0.0001, niter=1000):
  ...
  for i in range(niter):
      ...
      da = np.sum(2 * b * x - 2 * y + 2 * a)
      db = np.sum(2 * a * x -2 * y * x + 2 * b * (x ** 2))
...
def sse(x, y, param):
  ...
  y_chapeu = predict(x, param)
  variacao = y - y_chapeu
  soma_erros = np.sum(variacao**2)
...
def idade_universo(sd_x, sd_y):
  ...
  aux = sd_x/(sd_y*1000)
  idade_universo = aux / beta

  return idade_universo
