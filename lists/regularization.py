# -*- coding: utf-8 -*-
"""Regularização e viés de variância.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1GlhY5ds_26duVIwhEGGMb0wZSPGFG9_K
"""
...
def derivadas_regressao_reg(theta, X, y, beta):
    ...
    theta_der = -2 * ((y - X @ theta) @ X)
    theta_der[:-1] = theta_der[:-1] + 2 * (beta * theta[:-1])
...
def gd_reg(theta, d_fun, X, y, beta, lambda_=0.0001, tol=0.00001, max_iter=10000):
    ...
    while True:
        # Computar as derivadas
        grad = derivadas_regressao_reg(theta, X, y, beta)
        # Atualizar
        theta_novo = theta - lambda_ * grad
        
        # Parar quando o erro convergir
        err_sq_l2 = ((X.dot(theta) - y) ** 2).mean() + beta*(theta@theta.T)
        if np.abs(old_err_sq - err_sq_l2) <= tol:
            break
        theta = theta_novo
        old_err_sq = err_sq_l2
        #print('Iter {}; theta = '.format(i+1), theta)
        i += 1
        if i == max_iter:
            break
...
model = RidgeCV(alphas=[9e-2, 2e-3, 3e-1, 1.9]).fit(X, y)
model.score(X, y)