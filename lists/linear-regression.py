...
Original file is located at
    https://colab.research.google.com/drive/13TvkpcOTxvykDNvGJ4Rw1oiY3a1EZyrX
...
alpha = 1
beta = 0.5
x = df['midparentHeight'].values.mean()
y = df['childHeight'].values.mean()
...
def mean_error(alpha, beta, x, y):
  ...
  erro = error(alpha, beta, x_media, y_media)
...
media_pais = df['midparentHeight'].values
media_filhos = df['childHeight'].values

inicio = 64
fim = 76

plt.scatter(media_pais, media_filhos, alpha=0.8, edgecolors='k', s=80)
plt.plot([inicio, fim], [alpha + beta*inicio, alpha + beta*fim], color = 'red')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Regressão linear simples')
despine()
...
def sum_of_squared_errors(alpha, beta, x, y):
  ...
  aux_y = y - y.mean()
  aux_x = x - x.mean()
  y_predict = alpha + beta*aux_x

  erros =  aux_y - y_predict
  erros_quadrados = (erros.T).dot(erros)

  erro = np.sqrt(erros_quadrados)
...
def least_squares_fit(x, y):
  ...
  beta = (x_norm.dot(y_norm.T))/(x_norm.T@x_norm)
...
def r_squared(alpha, beta, x, y):
  ...
  y_aux = y - y_predict
  r_2 = 1 - (y_aux.T@y_aux)/(y_norm.T@y_norm)
