...
Original file is located at
    https://colab.research.google.com/drive/15358XJYRjoJrY_sS3oE_287kqmlqqzml
...
def delay(df) -> tuple:
  ...
  return (df_aux['DepDelay'].mean(), df_aux['ArrDelay'].mean())
...
def high_delay(df):
    df_novo = df.dropna()
    df_realizados = df_novo[df_novo["Cancelled"] == 0].copy()
  
    df_realizados.loc[df_realizados.CRSArrTime >= df_realizados.CRSDepTime, "Tempo de voo"] = df_realizados["CRSArrTime"] - df_realizados["CRSDepTime"]
    df_realizados.loc[df_realizados.CRSArrTime < df_realizados.CRSDepTime, "Tempo de voo"] = 2400 + df_realizados["CRSArrTime"] - df_realizados["CRSDepTime"]

    df_rotas = df_realizados.groupby(["OriginAirportName", "DestAirportName"]).mean()

    maior_voo = df_rotas["Tempo de voo"].idxmax()

    return maior_voo[0], maior_voo[1]
...
df_box = df[df["Cancelled"] == 0]
df_box.boxplot(column='DepDelay', by='DayOfWeek', grid=False, showfliers=False)
...
df_correlacao = df[df["Cancelled"] == 0]
plt.scatter(df_correlacao.DepDelay, df_correlacao.ArrDelay, edgecolor='k', alpha=0.75)
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')
